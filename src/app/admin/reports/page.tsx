...
export default async function ReportsPage() {
  ...
  const stageMap = Object.fromEntries(stageCounts.map(s => [s.stage, s._count._all]))
  ...
  const researcherIds = researcherStats.map(r => r.ownerId!).filter(Boolean)
  const researchers   = researcherIds.length
    ? await db.user.findMany({ where: { id: { in: researcherIds } }, select: { id: true, name: true } })
    : []
  const researcherMap = Object.fromEntries(researchers.map(r => [r.id, r.name ?? r.id]))
